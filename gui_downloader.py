...
from downloader import (
    ...
    console_log_func,  # Import console_log_func to pass to core functions
    console_input_func,  # Import console_input_func to pass to core functions
)
...
class TelegramDownloaderGUI:
    def __init__(self):
        ...
        self.upload_thread = None  # For upload operations
        self.is_downloading = False
        self.is_uploading = False  # New flag for upload
    ...
    def gui_get_input(self, prompt: str, default: Optional[str] = None, hide_input: bool = False) -> str:
        ...
        def show_input_dialog():
            ...
            if hide_input:
                entry.configure(show="*")  # Mask input for passwords
    ...
    def show_login_screen(self):
        ...
        title = ctk.CTkLabel(
            header,
            text="TELEGRAM MEDIA TOOL",  # Updated title
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Multi-account • Download • Upload • Auto-resume • Fast",  # Updated subtitle
    ...
    def show_source_screen(self):
        ...
        ctk.CTkLabel(
            header_frame,
            text="Select Operation / Source",  # Updated title
        ...
        self.left_panel.grid_rowconfigure(6, weight=1)  # Adjusted for new button

        ctk.CTkLabel(self.left_panel, text="OPERATIONS", font=ctk.CTkFont(size=16, weight="bold"),  # Updated label
                     text_color=self.colors['text']).pack(pady=(15, 10))

        # Buttons for source selection (these will trigger right panel updates)
        self.source_buttons = {}
        button_defs = [
            ("Upload Media", "upload", self.colors['upload_color']),  # NEW UPLOAD BUTTON
    ...
    def _display_continue_last_options_panel(self):
        ...
        for line_text in self.downloader.state.get_status_lines(self.downloader.download_dir):  # Pass download_dir
    ...
    def _display_upload_options_panel(self):
        ...
        ctk.CTkLabel(self.right_panel, text="Upload Media", font=ctk.CTkFont(size=18, weight="bold"),
                     text_color=self.colors['text']).pack(pady=(20, 10))
        ctk.CTkLabel(self.right_panel, text="Select a file and a destination chat/channel to upload.",
                     text_color=self.colors['text_dim'], wraplength=400).pack(pady=(0, 20))

        # File selection
        file_frame = ctk.CTkFrame(self.right_panel, fg_color="transparent")
        file_frame.pack(fill="x", padx=20, pady=(10, 5))
        ctk.CTkLabel(file_frame, text="File to Upload:", text_color=self.colors['text']).pack(anchor="w")
        self.upload_file_entry = ctk.CTkEntry(file_frame, fg_color=self.colors['card'],
                                              border_color=self.colors['upload_color'])
        self.upload_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
        ctk.CTkButton(file_frame, text="Browse", command=self._browse_upload_file,
                      fg_color=self.colors['card'], border_width=1, border_color=self.colors['upload_color']).pack(
            side="right")

        # Destination Dialog selection (ID or Username)
        dialog_selection_frame = ctk.CTkFrame(self.right_panel, fg_color="transparent")
        dialog_selection_frame.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(dialog_selection_frame, text="Destination (Chat ID, @Username, or Phone):",
                     text_color=self.colors['text']).pack(anchor="w")
        self.upload_destination_entry = ctk.CTkEntry(dialog_selection_frame,
                                                     placeholder_text="@username or 123456789 or +1234567890",
                                                     fg_color=self.colors['card'],
                                                     border_color=self.colors['upload_color'])
        self.upload_destination_entry.pack(fill="x")
    ...
    def _handle_start_upload(self):
        file_path_str = self.upload_file_entry.get().strip()
        destination_str = self.upload_destination_entry.get().strip()
        ...
        if not file_path_str or not Path(file_path_str).is_file():
            messagebox.showwarning("Missing File", "Please select a valid file to upload.")
            return
        if not destination_str:
            messagebox.showwarning("Missing Destination",
                                   "Please enter a destination (chat ID, username, or phone number).")
            return

        file_path = Path(file_path_str)
        self.is_uploading = True  # Set upload flag

        # Start upload in a separate thread
        self.upload_thread = threading.Thread(
            target=self._upload_thread_run,
            args=(destination_str, file_path, caption),
            daemon=True
        )
        self.upload_thread.start()

    def _upload_thread_run(self, destination: str, file_path: Path, caption: Optional[str]):
        ...
        try:
            ...
            def update_ui_progress(progress_percentage, current_bytes, total_bytes):
            ...
            self.root.after(0,
                            lambda: messagebox.showinfo("Upload Complete", f"Successfully uploaded {file_path.name}."))
            self.root.after(0, lambda: self.upload_progress_label.configure(text="Upload complete!"))
            self.is_uploading = False  # Reset flag on completion
    ...
    def _fetch_dialogs_thread_for_panel(self):
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
            dialogs = loop.run_until_complete(self.downloader.list_dialogs())  # Calls core list_dialogs
    ...
    def _render_dialog_list_in_panel(self, dialogs_to_display: List[Dict[str, Any]], target_frame):
        ...
        for dialog in dialogs_to_display:
            dialog_frame = ctk.CTkFrame(target_frame, fg_color=self.colors['bg'])
    ...
    def _on_dialog_search_key_release(self, event=None):
        ...
        if search_term:
            filtered_dialogs = [
                ...
                if search_term in d['title'].lower() or search_term in d['etype'].lower()
            ]
        else:
    ...
    def handle_login(self):
        ...
        # Find next available index for a new account (or reuse if user selected it interactively via CLI logic, but for GUI add, always new index)
        ...
        def login_in_thread():
            try:
                ...
                asyncio.set_event_loop(loop)
                # Call the do_login_flow from downloader.py
                updated_envd, logged_in_idx = loop.run_until_complete(
                    do_login_flow(
                        self.envd,
                        self.gui_log_output,  # Pass GUI's log function
                        self.gui_get_input,  # Pass GUI's input function
                        phone=phone,
                        api_id=api_id_int,
                        api_hash=api_hash,
                        download_dir=download_dir,
                        account_idx_to_use=new_idx  # Suggest a new index for new account
                    )
                )
                ...
                self.root.after(0, self.show_login_screen)  # Go back to login screen on error
    ...
    def handle_logout_current(self):
        ...
        if messagebox.askyesno("Confirm Logout", f"Logout account #{self.current_account_idx}?"):
            def logout_in_thread():
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                # Call do_logout_flow from downloader.py
    ...
    def handle_logout_and_return(self):
        """Logout và quay về login screen (từ source screen)"""
        if messagebox.askyesno("Confirm", "Logout and return to login screen?"):
            def logout_in_thread():
                ...
                if self.current_account_idx != 0 and self.downloader:
                    ...
                    updated_envd = self.envd.copy()  # No active account to logout, just use current envd
                self.root.after(0, lambda: self._post_logout_process(updated_envd))
    ...
    def _post_logout_process(self, updated_envd: Dict[str, str]):
        self.envd = updated_envd
        save_env(self.env_path, self.envd)
        self.current_account_idx = get_current_account_index(self.envd)  # Re-read current index after logout
    ...
    def handle_reset(self):
        ...
        if messagebox.askyesno("Confirm Reset",
            ...
            def reset_in_thread():
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                # Call do_reset_flow from downloader.py, pass confirm=True
                updated_envd = loop.run_until_complete(
    ...
    def _post_reset_process(self, updated_envd: Dict[str, str]):
        self.envd = updated_envd
        save_env(self.env_path, self.envd)
        self.current_account_idx = get_current_account_index(self.envd)  # Re-read current index
    ...
    def _scan_media_thread_for_panel(self):
        ...
        try:
            ...
            # Call _run_with_source which handles scanning, hashing, state management, and filter screen transition
            # It also handles the 'confirm_callback' for hash mismatch logic.
            # We pass GUI's progress callbacks
            success = loop.run_until_complete(
                self.downloader._run_with_source(
                    ...
                    progress_callback_download=None  # Download happens on separate screen
                )
            )

            if success:
                # After _run_with_source, self.downloader.stats and self.downloader.media_list are updated.
                # Now transition to filter screen
                self.media_list = self.downloader.media_list  # Ensure GUI has the full list from downloader
                self.stats = self.downloader.stats.copy()  # Ensure GUI has stats
                self.root.after(0, self.show_filter_screen)
            else:
                self.root.after(0,
                                self.show_source_screen)  # Go back to source selection on scan failure or user cancellation
    ...
    def _initiate_continue_session(self):
        ...
        # Thread to restore dialog entities if needed, then call _scan_media_thread_for_panel
        def _restore_dialogs_and_scan_thread():
            try:
                ...
                self.root.after(0, self._scan_media_thread_for_panel)  # Continue with scan after restoring dialogs
    ...
    def start_download(self, filter_choice):
        ...
        # Filter media list (already done in _run_with_source, but for GUI it's re-filtered)
        if filter_choice == "1":
        ...
        # Update state manager with last filter choice (this was already done in _run_with_source, but ensures it's current)
    ...
    def _download_thread(self):
        ...
        try:
            ...
            # Call downloader.download_all_media with GUI's progress callback
            loop.run_until_complete(
    ...
    def on_closing(self):
        ...
        if self.is_downloading or self.is_uploading:  # Check both flags
            if messagebox.askyesno("Confirm Exit", "Operation in progress. Exit anyway?"):
                self.stop_flag = True  # Will stop download and upload if running
                self._disconnect_and_close_loop()
                self.root.destroy()
        else:
            self._disconnect_and_close_loop()
